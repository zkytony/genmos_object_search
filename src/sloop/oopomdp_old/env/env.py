...
class MosEnvironment(pomdp_py.Environment):
    ...
    def __init__(self, dim, init_state, sensors, obstacles=set({}),
        ...
        reward_model = GoalRewardModel(self.target_objects, small=reward_small)
...
def parse_worldstr(worldstr):
    """Return worldlines and sensorlines from worldstr"""
    worldlines = []
    sensorlines = []
    mode = "world"
    ...
    return worldlines, sensorlines

# Interpret environment from world string.
def interpret(worldstr, no_look=False, reward_small=1, obj_id_map={}):
    """
